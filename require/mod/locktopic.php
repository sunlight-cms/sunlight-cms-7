...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $query = DB::query("SELECT id,author,time,subject,locked FROM `" . _mysql_prefix . "-posts` WHERE id=" . $id . " AND type=5 AND xhome=-1");
    if (DB::size($query) != 0) {
        $query = DB::row($query);
...
if ($continue && isset($_POST['doit'])) {
    ...
    DB::query("UPDATE `" . _mysql_prefix . "-posts` SET locked=" . (($query['locked'] == 1) ? 0 : 1) . " WHERE id=" . $id);
