...
$module .= '
  ...
  ';
$query = DB::query("SELECT id,title FROM `" . _mysql_prefix . "-groups` WHERE id!=2 ORDER BY level DESC");
...
$query = DB::query("SELECT `" . _mysql_prefix . "-users`.username, `" . _mysql_prefix . "-groups`.title, `" . _mysql_prefix . "-groups`.icon, `" . _mysql_prefix . "-users`.id FROM `" . _mysql_prefix . "-users`, `" . _mysql_prefix . "-groups` WHERE `" . _mysql_prefix . "-users`.`group`=`" . _mysql_prefix . "-groups`.id" . $grouplimit . " ORDER BY `" . _mysql_prefix . "-groups`.level DESC,`" . _mysql_prefix . "-users`.id " . $paging[1]);
while ($item = DB::row($query)) {
    $module .= "<tr><td>" . (($item['icon'] != "") ? "<img src='" . _indexroot . "pictures/groupicons/" . $item['icon'] . "' alt='icon' class='icon' /> " : '') . "<a href='index.php?m=profile&amp;id=" . $item['username'] . "'>" . $item['username'] . "</a></td><td>" . $item['title'] . "</td></tr>";
}
$module .= "</table>";
